...
单个任务不需要二次确认；多个任务必须确认后才修改。没有足够证据、目标不唯一或结果集不完整时，都会先停止，不强行猜测。

## 一个案例：它会把什么变好

<p align="center">
  <img src="./assets/readme/title-example.svg" width="100%" alt="说明性案例：从泛化任务名称提炼为带日期、类型和具体主题的侧边栏标题">
</p>

这是说明性案例，不代表某个真实任务的处理结果。实际标题会以任务的 `createdAt`、用户目标和最终交付为依据。
...
- 批量整理默认排除发起本次整理的当前任务；无范围指向的默认单任务整理则包含当前任务。
...
| 最终修改数量 | 行为 |
| --- | --- |
| 1 个明确任务 | 读取并判断后直接改名，随后报告结果 |
| 2 个及以上任务 | 先输出 `| 原名称 | 新名称 |`，用户确认后再批量改名 |
| 目标不唯一或结果集不完整 | 先澄清或停止，不执行改名 |
...
| 范围 | 匹配方式 |
| --- | --- |
| 当前任务（默认） | 只调用 Skill 或没有其他范围指向时，使用当前任务 ID |
| 单个任务 | 使用任务 ID，或先定位后确认唯一任务 |
